...
local function GrabObject()
    ...
    activeObject:sendEvent(D.e.HeldBy, { actor = omwself.object });
end
...
local function DropObject()
    ...
    activeObject:sendEvent(D.e.HeldBy, { actor = nil });
    activeObject = nil
end
...
local function HoldGrabbedObject(dt, ignoreCollisions)
    ...
    activeObject:sendEvent(D.e.SetPhysicsProperties, {
        ignorePhysObjectCollisions = ignoreCollisions,
        ignoreWorldCollisions = ignoreCollisions
    })
    activeObject:sendEvent(D.e.MoveTo, { position = objectHoldPos, maxImpulse = maxDragImpulse, culprit = omwself.object })
    ...
    if input.isMouseButtonPressed(1) then
        ...
        -- Launching!
        activeObject:sendEvent(D.e.ApplyImpulse, {impulse=direction*throwImpulse, culprit = omwself.object })
        DropObject()
    end
end
